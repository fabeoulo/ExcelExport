...
public class SendLackWithStock extends SendEmailBase {
    ...
    private List<Orders> checkedOrders;
    ...
    private List<Orders> getCheckedLack() throws Exception {
        //M3
        Integer teamId = 2;
        ...
        List<Requisition> lackReq = getReqFromLack(lackOrders);
        Map<String, BigDecimal> stockMap = sapService.getStockMapWithGoodLgort(lackReq);

        //FIFO
        List<Orders> checkedL = lackOrders.stream().filter(o -> {
    ...
    private List<Requisition> getReqFromLack(List<Orders> lackOrders) {
        if (lackOrders.isEmpty()) {
            return Arrays.asList();
        }
        List<Integer> listInt = lackOrders.stream().map(l -> l.getRequisionId()).collect(Collectors.toList());
        return rservice.findAllByIdWithUserAndState(listInt);
    }

    private String[] findEMailFromLack() {
        List<Requisition> lackReq = getReqFromLack(checkedOrders);
